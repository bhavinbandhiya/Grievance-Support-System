...
public partial class AdminPanel_GRI_Grievance_List : System.Web.UI.Page
{
    #region 11.0 Variables

    String FormName = "AdminPanel_GRI_Grievance_List";
    ...
    private void Search(int PageNo)
    {
        ...
        if (dtpGrievanceSystemFromDate.Text.Trim() != String.Empty)
			FromDate = Convert.ToDateTime(dtpGrievanceSystemFromDate.Text.Trim());

        if (dtpGrievanceSystemToDate.Text.Trim() != String.Empty)
			ToDate = Convert.ToDateTime(dtpGrievanceSystemToDate.Text.Trim());
		...
		GRI_GrievanceBAL balGRI_Grievance = new GRI_GrievanceBAL();
    ...
    protected void lbtnExport_Click(object sender, EventArgs e)
    {
        ...
        DataTable dt = new DataTable(); 
            //balGrievanceSystemDetails.SelectPage(Offset, PageRecordSize, out TotalReceivedRecord, Ticker, OrderTypeID, Strategy, EmotionEntryGrievanceSystem, EmotionExitGrievanceSystem, EmotionSLHit, Rating, GrievanceSystemFromDate, GrievanceSystemToDate, DayName, Convert.ToInt32(Session["UserID"]));
        if (dt != null && dt.Rows.Count > 0)
        {
